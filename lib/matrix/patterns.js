...
const setupPatterns = (matrix, version) => {
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    const isLastCol = i === positions.length - 1;

    const startY = isFirstCol || isLastCol ? 1 : 0;
    const endY = isFirstCol ? positions.length - 1 : positions.length;

    for (let j = startY; j < endY; j++) {
      placeAlignmentPattern(matrix, x, positions[j]);
    }
  }
