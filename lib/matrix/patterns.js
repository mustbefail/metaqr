...
const isFinderPatternPlace = (patternX, patternY, positions) => {
  const isTL = patternX === 0 && patternY === 0;
  const isTR = patternX === 0 && patternY === positions.length - 1;
  const isBL = patternX === positions.length - 1 && patternY === 0;
...
const setupPatterns = (matrix, version) => {
  ...
  for (let i = 0; i < positions.length; i++) {
    for (let j = 0; j < positions.length; j++) {
      ...
      // Skip finder patterns: Top-Left, Top-Right, Bottom-Left
      if (isFinderPatternPlace(i, j, positions)) continue;
