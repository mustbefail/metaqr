...
const getBlockInfo = (version, eccLevel) => {
  ...
  if (!info) throw new TypeError(`Block info not available for version ${version}`);

  const blocks = info[eccLevel];

  if (!blocks) throw new TypeError(`ECC level ${eccLevel} not available for version ${version}`);
...
const getNsym = (version, eccLevel) => {
  ...
  if(!cap) throw new TypeError(`Version ${version} is out of range (1-40)`);
...
const getDataCapacity = (version, eccLevel) => {
  ...
  if (!spec) throw new TypeError(`Version ${version} is out of range (1-40)`);
...
const getFormatInfo = (eccLevel, maskPattern) => {
  ...
  if (!group) throw new TypeError(`ECC level ${eccLevel} is out of range (0-3)`);
  if (maskPattern < 0 || maskPattern >= group.length) throw new TypeError(`Invalid mask pattern: ${maskPattern}`);
