...
class BitBuffer {
  ...
  concat(buffer) {
    if (!(buffer instanceof BitBuffer)) {
      throw new TypeError('Expected a BitBuffer instance');
    }

    this.#extendCapacity(this.#bitLength + buffer.#bitLength);

    if (this.#bitLength % 8 === 0) {
      const srcByteCount = Math.ceil(buffer.#bitLength / 8);
      const destByteOffset = this.#bitLength / 8;
      this.#bytes.set(buffer.#bytes.subarray(0, srcByteCount), destByteOffset);
      this.#bitLength += buffer.#bitLength;
      return this;
    }

    for (let i = 0; i < buffer.#bitLength; i++) {
      const bit = (buffer.#bytes[Math.floor(i / 8)] >> (7 - (i % 8))) & 1;
      if (bit === 1) {
        const byteIndex = Math.floor(this.#bitLength / 8);
        const bitPos = 7 - (this.#bitLength % 8);
        this.#bytes[byteIndex] |= 1 << bitPos;
      }
      this.#bitLength++;
    }
