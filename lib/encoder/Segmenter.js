...
class Segmenter {
  ...
  static #segmentWithMode(text, mode) {
    const modes = {
      [MODE_INDICATORS.NUMERIC]: () => new Numeric(text),
      [MODE_INDICATORS.ALPHANUMERIC]: () => new Alphanumeric(text),
      [MODE_INDICATORS.BYTE]: () => new Byte(text),
    };
  ...
  static #segmentAuto(text) {
    ...
    for (const segment of rawSegments) {
      if (
        segment.type === SEGMENT_TYPES.NUMERIC &&
        segment.text.length < THRESHOLD_NUMERIC
      ) {
        segment.type = SEGMENT_TYPES.ALPHANUMERIC;
      }
    }

    const merged = Segmenter.#mergeAdjacent(rawSegments);

    for (const segment of merged) {
      if (
        segment.type === SEGMENT_TYPES.ALPHANUMERIC &&
        segment.text.length < THRESHOLD_ALPHANUMERIC
      ) {
        segment.type = SEGMENT_TYPES.BYTE;
      }
    }

    return Segmenter.#mergeAdjacent(merged).map(({ type, text }) =>
      SEGMENTS_MAP[type](text),
    );
  }
